...
import { MessageSquare, Plus, Search, Mail, ChevronLeft, Users, RefreshCw, Send } from 'lucide-react';
import { GBtn } from '../ui/GBtn';
import { GInput } from '../ui/GInput';
import { GTxt } from '../ui/GTxt';
import { Modal } from '../ui/Modal';
import { twilioSend, pollTwilioIncoming, sendEmail } from '../../lib/messaging';
import { uid } from '../../lib/utils';

export function InboxPage({ threads = [], setThreads, customers = [], settings = {}, toast }: any) {
  const [active, setActive] = useState<any>(threads[0]?.id || null);
  ...
  const activeThread = threads.find((t: any) => t.id === active);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      setPolling(true);
      try {
        const lastTs = Math.max(0, ...threads.flatMap((t: any) => t.messages.map((m: any) => m.ts)));
        ...
        if (incoming.length > 0) {
          setThreads((prev: any[]) => {
            ...
            incoming.forEach((msg: any) => {
              ...
              const customer = customers.find((c: any) => c.phone?.replace(/\D/g, "") === phone.replace(/\D/g, ""));
          ...
          if (incoming.length > 0 && toast) toast(incoming.length + " new message" + (incoming.length > 1 ? "s" : ""));
        }
  ...
  const markRead = (id: string) => setThreads(threads.map((t: any) => t.id === id ? { ...t, unread: false } : t));

  const send = async () => {
    if (!input.trim() || !activeThread || sending) return;
    const msgText = input.trim();
    const outMsg = { id: uid(), dir: "out", body: msgText, ts: Date.now(), status: "sending" };
    setThreads((prev: any[]) => prev.map(t => t.id === active ? { ...t, messages: [...t.messages, outMsg] } : t));
    setInput("");
    setSending(true);
    try {
      if (activeThread.channel === "sms") {
        ...
        setThreads((prev: any[]) => prev.map(t => t.id === active ? { ...t, messages: t.messages.map(m => m.id === outMsg.id ? { ...m, status: "sent" } : m) } : t));
        if (toast) toast("SMS sent ✓");
      } else {
        if (!activeThread.contactEmail) throw new Error("No email for this contact");
        const lastSubject = activeThread.messages.find((m: any) => m.subject)?.subject || "";
        await sendEmail(settings, { to: activeThread.contactEmail, subject: "Re: " + lastSubject, body: msgText });
        setThreads((prev: any[]) => prev.map(t => t.id === active ? { ...t, messages: t.messages.map(m => m.id === outMsg.id ? { ...m, status: "sent" } : m) } : t));
        if (toast) toast("Email sent ✓");
      }
    } catch (err: any) {
      setThreads((prev: any[]) => prev.map(t => t.id === active ? { ...t, messages: t.messages.map(m => m.id === outMsg.id ? { ...m, status: "failed", error: err.message } : m) } : t));
      if (toast) toast("Send failed: " + err.message, "error");
    } finally { setSending(false); }
  };

  const startNew = async () => {
    if (!newDraft.to.trim() || !newDraft.body.trim()) return;
    const customer = customers.find((c: any) => c.firstName + " " + c.lastName === newDraft.to.trim() || c.phone?.includes(newDraft.phone) || c.email === newDraft.email);
    if (newDraft.channel === "sms" && !newDraft.phone) { if (toast) toast("Enter a phone number", "error"); return; }
    if (newDraft.channel === "email" && !newDraft.email) { if (toast) toast("Enter an email", "error"); return; }
    ...
    try {
      ...
      setThreads((prev: any[]) => prev.map(t => t.id === newThread.id ? { ...t, messages: t.messages.map(m => m.id === outMsg.id ? { ...m, status: "sent" } : m) } : t));
      if (toast) toast("Sent ✓");
    } catch (err: any) {
      setThreads((prev: any[]) => prev.map(t => t.id === newThread.id ? { ...t, messages: t.messages.map(m => m.id === outMsg.id ? { ...m, status: "failed", error: err.message } : m) } : t));
      if (toast) toast("Send failed: " + err.message + (err.message.includes("Twilio not configured") ? " — add Twilio credentials in Settings" : ""), "error");
    }
    setNewDraft({ channel: "sms", to: "", phone: "", email: "", subject: "", body: "" });
  };

  const filteredThreads = threads.filter((t: any) => !search || t.contactName.toLowerCase().includes(search.toLowerCase()) || t.messages.some((m: any) => m.body.toLowerCase().includes(search.toLowerCase())));
  ...
  const relTime = (ts: number) => { const s = Math.floor((Date.now() - ts) / 1000); if (s < 60) return "now"; if (s < 3600) return Math.floor(s / 60) + "m"; if (s < 86400) return Math.floor(s / 3600) + "h"; return Math.floor(s / 86400) + "d"; };
    ...
    <div className="flex -mx-4 md:-mx-6 -my-4 bg-black overflow-hidden rounded-xl border border-red-900/30" style={{ height: "calc(100vh - 57px)" }}>
      <div className="w-full md:w-80 border-r border-red-900/30 flex flex-col flex-shrink-0" style={{ display: activeThread && window.innerWidth < 768 ? "none" : "flex" }}>
        <div className="p-3 border-b border-red-900/30 space-y-2">
          <div className="flex items-center justify-between">
            <div className="font-semibold flex items-center gap-2"><MessageSquare size={14} className="text-red-400" />Inbox <span className="text-[10px] text-white/50">{threads.filter((t: any) => t.unread).length > 0 ? threads.filter((t: any) => t.unread).length + " unread" : "all read"}</span></div>
      ...
      <Modal open={newModal} onClose={() => setNewModal(false)} title="New Message" maxW="max-w-lg">
        <div className="space-y-3">
          ...
          <GInput placeholder="Contact name" value={newDraft.to} onChange={(e: any) => { const c = customers.find((x: any) => (x.firstName + " " + x.lastName).toLowerCase().startsWith(e.target.value.toLowerCase())); setNewDraft({ ...newDraft, to: e.target.value, phone: c?.phone || newDraft.phone, email: c?.email || newDraft.email }); }} list="contact-names" />
          <datalist id="contact-names">{customers.map((c: any) => <option key={c.id} value={c.firstName + " " + c.lastName} />)}</datalist>
          {newDraft.channel === "sms" && <GInput placeholder="Phone (+15551234567)" value={newDraft.phone} onChange={(e: any) => setNewDraft({ ...newDraft, phone: e.target.value })} />}
          {newDraft.channel === "email" && <>
            <GInput placeholder="Email address" value={newDraft.email} onChange={(e: any) => setNewDraft({ ...newDraft, email: e.target.value })} />
            <GInput placeholder="Subject" value={newDraft.subject} onChange={(e: any) => setNewDraft({ ...newDraft, subject: e.target.value })} />
          </>}
          <GTxt rows={4} placeholder="Message..." value={newDraft.body} onChange={(e: any) => setNewDraft({ ...newDraft, body: e.target.value })} />
