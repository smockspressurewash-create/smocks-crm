...
import { AlertTriangle, Clock, Phone, MessageSquare, Mail, RefreshCw } from 'lucide-react';
...
export function PipelinePage({ 
  ...
  const avgDays = pipelineStages.map(stg => {
    ...
    const days = sj.map((j: any) => daysSince(j.stageChangedAt || j.scheduledDate));
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        ...
        const stale = filtered.filter((j: any) => activeStages.includes(j.pipelineStage) && daysSince(j.stageChangedAt || j.scheduledDate || j.createdAt) >= staleThreshold);
          ...
          <Glass className="p-4 !bg-orange-950/20 !border-orange-700/40">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {stale.map((j: any) => {
                    const cu = customers.find((c: any) => c.id === j.customerId);
                    ...
                    return <tr key={j.id} className="border-b border-orange-900/20 hover:bg-orange-950/10">
                      ...
                      <td className="py-2 text-center"><Badge tone="orange">{(pipelineStages.find(s => s.key === j.pipelineStage) as any)?.label || j.pipelineStage}</Badge></td>
                  ...
                  {sj.map((j: any) => {
                    ...
                    return (
                      <SwipeableCard key={j.id} job={j} stages={pipelineStages} onMove={moveStg}>
                        <div draggable onDragStart={() => setDragId(j.id as any)} className={"p-3 bg-black/60 rounded-xl cursor-grab hover:border active:cursor-grabbing select-none border " + stg.border + " hover:" + stg.border}>
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1 flex-wrap">
                                ...
                                {j.priority && j.priority !== "normal" && <Badge tone={(prio as any).tone}>{prio.label}</Badge>}
                              </div>
                              <div className="text-[11px] text-white/50 truncate">{j.address}</div>
                              {j.tags && j.tags.length > 0 && <div className="flex gap-1 mt-1 flex-wrap">{j.tags.slice(0, 2).map((t: string) => <span key={t} className="text-[8px] px-1.5 py-0.5 rounded-full bg-white/10 text-white/70">{t}</span>)}</div>}
                              <div className="flex items-center justify-between mt-1">
                          ...
                          <div className="flex gap-1 mt-2 pt-2 border-t border-white/5">
                            ...
                            {stg.key === "lost" ? (
                              <button onClick={() => { moveStg(j.id, "contacted"); if (toast) toast("Lead re-opened → Contacted"); }} className="flex-1 p-1 rounded hover:bg-green-900/30 text-white/50 hover:text-green-400 flex items-center justify-center text-[9px] font-semibold gap-0.5"><RefreshCw size={9} />Re-open</button>
                            ) : (
      ...
      <Modal open={!!lostJob} onClose={() => setLostJob(null)} title="Why lost?">
        <div className="space-y-3">
          <GSel value={lostReason} onChange={(e: any) => setLostReason(e.target.value)}>
