    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <GBtn variant="ghost" className="!text-xs" onClick={() => {
            const html = `<!DOCTYPE html><html><head><title>Crew Boss Report</title><style>body{font-family:Arial;padding:32px;color:#111;max-width:900px;margin:auto}h1{color:#dc2626}table{width:100%;border-collapse:collapse;margin:16px 0;font-size:12px}th{background:#f0f0f0;padding:8px;text-align:left;border-bottom:2px solid #ccc;text-transform:uppercase;font-size:10px}td{padding:7px;border-bottom:1px solid #eee}.num{text-align:right}.kpi{display:inline-block;padding:12px 20px;margin:8px;border:1px solid #ddd;border-radius:8px;text-align:center;min-width:100px}.kv{font-size:22px;font-weight:bold;color:#dc2626}.kl{font-size:10px;color:#888;text-transform:uppercase}</style></head><body><h1>Crew Boss</h1><p style="color:#666">${tfLabel} Report · ${today()}</p><div style="margin:16px 0">${[["Revenue",fmt(totalRev)],["Jobs",tfJobs.length],["Close Rate",cr+"%"],["Profit",fmt(gp)]].map(([l,v])=>`<div class="kpi"><div class="kv">${v}</div><div class="kl">${l}</div></div>`).join("")}</div><h2>Job Breakdown</h2><table><thead><tr><th>Date</th><th>Customer</th><th>Address</th><th class="num">Amount</th><th>Status</th></tr></thead><tbody>${tfJobs.slice(0,100).map(j=>{const c=customers.find(x=>x.id===j.customerId);return`<tr><td>${j.scheduledDate}</td><td>${c?c.firstName+" "+c.lastName:""}</td><td>${j.address?.split(",")[0]||""}</td><td class="num">$${j.amount}</td><td>${j.status}</td></tr>`;}).join("")}</tbody></table><p style="font-size:10px;color:#999;margin-top:24px">Crew Boss · York, PA</p><script>window.onload=()=>setTimeout(window.print,300)<\/script></body></html>`;
