...
import { Plus, Layers, Save, Trash2, Shield, Clipboard, Lock, X } from 'lucide-react';
...
import { fmt, uid, today, daysFromNow } from '../../lib/utils';

export function EstimateBuilder({ open, onClose, customers = [], services = [], settings = {}, onSave, estimateTemplates = [], setEstimateTemplates = () => { } }: any) {
  const [cid, setCid] = useState("");
  const [items, setItems] = useState<any[]>([]);
  const [vu, setVu] = useState(daysFromNow(30));
  const [discount, setDiscount] = useState<any>(0);
  const [depositRequired, setDepositRequired] = useState<any>(0);
  ...
  const loadTemplate = (tpl: any) => {
    setItems(tpl.lineItems.map((li: any) => ({ ...li, id: uid() })));
  ...
  const saveAsTemplate = () => {
    ...
    setEstimateTemplates((prev: any[]) => [...prev, tpl]);
    setSavingTemplate(false);
    setTemplateName("");
  };
    ...
    <Modal open={open} onClose={onClose} title="Build Estimate" maxW="max-w-3xl">
      <div className="space-y-4">
        <div className="grid md:grid-cols-2 gap-3">
          <div><label className="text-xs text-white/60 mb-1 block">Customer</label><GSel value={cid} onChange={(e: any) => setCid(e.target.value)}>{customers.map((c: any) => <option key={c.id} value={c.id} className="bg-black">{c.firstName} {c.lastName}</option>)}</GSel></div>
          <div><label className="text-xs text-white/60 mb-1 block">Valid until</label><GDate value={vu} onChange={(e: any) => setVu(e.target.value)} /></div>
        </div>
        ...
        {estimateTemplates.length > 0 && <div className="flex items-center gap-2 p-3 bg-purple-950/20 border border-purple-700/30 rounded-xl">
          ...
          <GSel onChange={(e: any) => { if (e.target.value) { const t = estimateTemplates.find((x: any) => x.id === e.target.value); if (t) loadTemplate(t); e.target.value = ""; } }} value="" className="!text-xs flex-1">
        ...
        <div className="p-3 bg-black/40 border border-red-900/20 rounded-xl">
          ...
          <GSel onChange={(e: any) => e.target.value && addSvc(e.target.value)} value=""><option value="" className="bg-black">Select service...</option>{services.map((s: any) => <option key={s.id} value={s.id} className="bg-black">{s.name} — {fmt(s.price)}</option>)}</GSel>
        </div>
        ...
        {(() => {
          ...
          const marketAvg: any = { "House Soft Wash": 385, "Driveway Wash": 175, "Roof Soft Wash": 520, "Deck / Patio Wash": 245, "Fence Wash": 175, "Gutter Cleaning": 159 };
        ...
        <div>
          ...
          {savingTemplate && <div className="flex gap-2 mb-2 p-2.5 bg-purple-950/20 border border-purple-700/30 rounded-xl">
            <GInput placeholder="Template name (e.g. House + Driveway Package)" value={templateName} onChange={(e: any) => setTemplateName(e.target.value)} onKeyDown={(e: any) => e.key === "Enter" && saveAsTemplate()} className="!text-xs flex-1" />
          ...
          <div className="space-y-2">
            {items.map(it => (
              <div key={it.id} className="grid grid-cols-12 gap-2 items-center">
                <div className="col-span-12 md:col-span-6"><GInput placeholder="Description" value={it.description} onChange={(e: any) => setItems(items.map(i => i.id === it.id ? { ...i, description: e.target.value } : i))} /></div>
                <div className="col-span-4 md:col-span-2"><GInput type="number" placeholder="Qty" value={it.quantity} onChange={(e: any) => setItems(items.map(i => i.id === it.id ? { ...i, quantity: e.target.value } : i))} /></div>
                <div className="col-span-6 md:col-span-3"><GInput type="number" placeholder="Price" value={it.unitPrice} onChange={(e: any) => setItems(items.map(i => i.id === it.id ? { ...i, unitPrice: e.target.value } : i))} /></div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-xs text-white/60 mb-1 block">Discount ($)</label><GInput type="number" step="0.01" value={discount} onChange={(e: any) => setDiscount(e.target.value)} /></div>
          <div><label className="text-xs text-white/60 mb-1 block">Deposit Required ($)</label><GInput type="number" step="0.01" value={depositRequired} onChange={(e: any) => setDepositRequired(e.target.value)} /></div>
        </div>

        <div className="grid md:grid-cols-2 gap-3">
          <div><label className="text-xs text-white/60 mb-1 block flex items-center gap-1"><Shield size={10} />Terms & Conditions</label><GTxt rows={3} value={terms} onChange={(e: any) => setTerms(e.target.value)} placeholder="Payment terms, cancellation policy..." className="!text-xs" /></div>
          <div><label className="text-xs text-white/60 mb-1 block flex items-center gap-1"><Clipboard size={10} />Customer Notes <span className="text-white/30 font-normal">(visible on estimate)</span></label><GTxt rows={2} value={notes} onChange={(e: any) => setNotes(e.target.value)} placeholder="e.g. 'We'll pre-treat the algae stains on the north side'" className="!text-xs" /></div>
          <div><label className="text-xs text-white/60 mb-1 block flex items-center gap-1"><Lock size={10} className="text-yellow-400" />Internal Notes <span className="text-white/30 font-normal">(crew only, not shown to customer)</span></label><GTxt rows={2} value={internalNote} onChange={(e: any) => setInternalNote(e.target.value)} placeholder="Gate code, dog warning, access issues, chemical notes..." className="!text-xs border-yellow-900/30" /></div>
        </div>
