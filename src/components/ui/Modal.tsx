...
export const Modal = ({ open, onClose, title, children, maxW = "max-w-lg", noBodyScroll = false }) => {
  ...
  const bgStyle = {
    ...
    backdropFilter: visible ? "blur(10px) saturate(0.7)" : "blur(0px)",
    WebkitBackdropFilter: visible ? "blur(10px) saturate(0.7)" : "blur(0px)",
  ...
  return (
    <>
      {/* Backdrop — separate element so backdrop-filter isn't affected by overflow */}
      <div
      ...
      {/* Scroll container — transparent, sits above backdrop */}
      <div
