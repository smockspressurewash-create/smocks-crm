...
**RLS**: policies used so far are permissive (`FOR ALL USING (true) WITH CHECK (true)`) — this is a single-owner app, not scoped per-tenant. If a Supabase read returns 0 rows unexpectedly (especially `employees`), suspect RLS blocking the owner, not a data problem.
